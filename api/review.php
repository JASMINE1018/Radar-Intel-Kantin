...
$user_id = $isUserLoggedIn ? $_SESSION['user_id'] : $_SESSION['owner_id'];
...
if ($isUserLoggedIn) {
    ...
    $stmt->close();
    $userName = $urow['nama'] ?? 'User';
} else {
    $stmt = $conn->prepare("SELECT nama FROM owners WHERE id = ?");
    $stmt->bind_param('i', $user_id);
    $stmt->execute();
    $urow = $stmt->get_result()->fetch_assoc();
    $stmt->close();
    $userName = $urow['nama'] ?? 'Owner';
}
$conn->close();
